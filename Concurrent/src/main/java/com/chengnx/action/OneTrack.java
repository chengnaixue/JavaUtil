...
import com.chengnx.entity.Player;
import com.chengnx.entity.Result;

public class OneTrack {

	private static final String[] PLAYERNAMES = new String[] { "白银圣斗士", "黄金圣斗士", "青铜圣斗士", "神斗士", "冥斗士", "哈迪斯", "龟仙人",
			"孙悟空", "孙悟饭", "贝吉塔", "孙悟天" };
	...
	public void track() {
		 ...
		 * 赛跑分为以下几个阶段进行；
		 * 
		 * 1、报名 2、初赛，10名选手，分成两组，每组5名选手。 分两次进行初赛（因为场地只有5条赛道，只有拿到进场许可的才能使用赛道，进行比赛）
		 * 
		...
		for (int index = 0; index < 3; index++) {
			...
			switch (index) {
			case 0:
				System.out.println("第一名：" + player.getName() + "[" + player.getNumber() + "]，成绩："
						+ player.getResult().getTime() + "秒");
				break;
			case 1:
				System.out.println("第二名：" + player.getName() + "[" + player.getNumber() + "]，成绩："
						+ player.getResult().getTime() + "秒");
				break;
			case 2:
				System.out.println("第三名：" + player.getName() + "[" + player.getNumber() + "]，成绩："
						+ player.getResult().getTime() + "秒");
				break;
			default:
				break;
	...
	/**
	 * 这是计分线程，是为了保证产生比赛结果后，在计入PriorityBlockingQueue 这样才有排列成绩的依据
	 * 
	 * @author yinwenjie
	 *
	 */
	private class FutureThread extends Thread {
		...
		/**
		 * 跑步成绩出来后，需要操作的队列 （要将对应的选手加入到队列，以便依据成绩进行排序）
		 */
